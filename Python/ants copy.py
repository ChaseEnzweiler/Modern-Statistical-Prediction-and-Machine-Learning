    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                #currently bodyguard in space and wants to add 
                if (self.ant.container == True) and (self.ant.can_contain(insect)):
                ...
                #not a bodyguard ant in place and trying to add a bodyguard

                if (insect.container == True) and (insect.can_contain(self.ant)):
...
class Ant(Insect):
    ...
    container = False # question 7 bodyguard ant#######

    def __init__(self, armor=1):
        """Create an Ant with an ARMOR quantity."""
        Insect.__init__(self, armor)

        #Adding this to keep track of doubled damage for ants
...
class ThrowerAnt(Ant):
    ...
    min_range = 0
    max_range = 1000000000000 # there is no max for thrower
    def nearest_bee(self, hive):
        ...
        ant_spot_bees = self.place.bees

        counter = 0 

        while counter < self.min_range: #gets a starting point that starts at minimum shooting range

            ant_spot = ant_spot.entrance

            ant_spot_bees = ant_spot.bees

            counter += 1

        while (ant_spot.entrance is not hive) and (ant_spot_bees == []) and (counter < self.max_range): #exits loop when out of max_range
        ...
        return random_or_none(ant_spot_bees)
...
class FireAnt(Ant):
    ...
    damage = 3
    # BEGIN Problem 4A
    food_cost = 5 # this was a user replaced line "altered line"
    ...
    def reduce_armor(self, amount):
        ...
        Insect.reduce_armor(self, amount) #execute at the end or the place will be set to none
        # END Problem 4A
...
class WallAnt(Ant):
    ...
    def __init__(self):
        ...
        Ant.__init__(self, 4) #sets armor
...
class TankAnt(BodyguardAnt):
    ...
    #both the tank ant and the ant inside the tank attack
    def action(self, colony):
        # BEGIN Problem 8
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 9
        """ use while loop to go through all the places.exit and then instantize __init__
        to make it that it tracks when ants damage is doubled similiar to the queen class
        than can use conditio
        """
        ...
        def double_damage(place):
            ...
            if (place.ant is not None):
                ...
                if (place.ant.doubled == False):

                    place.ant.doubled = True

                    place.ant.damage *= 2

                if (place.ant.container == True) and (place.ant.ant is not None) and (place.ant.ant.doubled == False):
        ...
        toward_exit = self.place

        while toward_exit.exit is not None:

            toward_exit = toward_exit.exit

            double_damage(toward_exit)
    ...
    def reduce_armor(self, amount):
        ...
        if (self.imposter == False) and (amount >= self.armor):
